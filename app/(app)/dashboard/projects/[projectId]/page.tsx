import { AlertTriangle, FileText, FolderOpenDot, MessageSquareText } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Completion" value={`${project.completionPercentage}%`} detail="Overall execution progress" icon={<FolderOpenDot className="size-4" />} />
        <MetricCard label="Pending approvals" value={String(project.pendingApprovalsCount)} detail="Awaiting client or ops confirmation" icon={<AlertTriangle className="size-4" />} />
        <MetricCard label="Unread messages" value={String(project.unreadMessagesCount)} detail="Secure thread activity" icon={<MessageSquareText className="size-4" />} />
        <MetricCard label="Outstanding actions" value={String(project.outstandingActionsCount)} detail="Tasks needing attention" icon={<FileText className="size-4" />} />
      </div>

      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        {timeline.length ? (
          <ActivityFeed items={timeline.slice(0, 6)} title="Recent timeline activity" />
        ) : (
          <DashboardEmptyState
